...
HTTP_CODE=$(curl -s -o "$CURL_OUTPUT" -w "%{http_code}" -X POST \
  -H "Content-Type: application/json" -d '{}' "$PING_URL/reset" --max-time 30 2>"$CURL_OUTPUT" || printf "000")

if [ "$HTTP_CODE" = "200" ]; then pass "HF Space is live and responds to /reset"
elif [ "$HTTP_CODE" = "000" ]; then
  fail "HF Space not reachable"; hint "Is the server running on $PING_URL ?"; stop_at "Step 1"
else
  fail "HF Space /reset returned HTTP $HTTP_CODE (expected 200)"; stop_at "Step 1"
fi

log "${BOLD}Step 2/3: Running docker build${NC} ..."
if ! command -v docker &>/dev/null; then fail "docker not found"; stop_at "Step 2"; fi
...
if [ "$BUILD_OK" = true ]; then pass "Docker build succeeded"
  ...
  fail "Docker build failed"; printf "%s\n" "$BUILD_OUTPUT" | tail -20; stop_at "Step 2"
fi

log "${BOLD}Step 3/3: Running openenv validate${NC} ..."
OPENENV_CMD=""
for candidate in openenv /Users/harshit/Library/Python/3.14/bin/openenv; do
  if command -v "$candidate" &>/dev/null 2>&1; then OPENENV_CMD="$candidate"; break; fi
done
if [ -z "$OPENENV_CMD" ]; then fail "openenv command not found"; hint "pip install openenv-core"; stop_at "Step 3"; fi

VALIDATE_OK=false
VALIDATE_OUTPUT=$(cd "$REPO_DIR" && "$OPENENV_CMD" validate 2>&1) && VALIDATE_OK=true
if [ "$VALIDATE_OK" = true ]; then
  pass "openenv validate passed"; [ -n "$VALIDATE_OUTPUT" ] && log "  $VALIDATE_OUTPUT"
else
  fail "openenv validate failed"; printf "%s\n" "$VALIDATE_OUTPUT"; stop_at "Step 3"
fi
